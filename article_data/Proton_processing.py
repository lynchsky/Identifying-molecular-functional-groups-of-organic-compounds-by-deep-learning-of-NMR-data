...
def guess_udic(dic, data):
    ...
    udic = ng.fileiobase.create_blank_udic(1)##创建一个空的字典，记录谱图参数

    # update default values (currently only 1D possible)
    # "label"
    try:
        label_value = dic[".OBSERVENUCLEUS"][0].replace("^", "")####原始数据例子文件中：##.OBSERVE NUCLEUS=	^1H
        udic[0]["label"] = label_value ##例子中标签为1H
    except KeyError:
    ...
    obs_freq = None
    try:
        obs_freq = float(dic[".OBSERVEFREQUENCY"][0])####原始数据例子文件中：##.OBSERVE FREQUENCY=	400.13247096654299639
        udic[0]["obs"] = obs_freq##磁场频率

    except KeyError:
        pass

    # "size"
    if isinstance(data, list):###isinstance()函数来判断一个对象是否是一个已知的类型，判断data是否是list
...
def spectral_processing(file, datatype):##原始谱图中提取数据
