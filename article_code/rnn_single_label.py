...
##测试集训练集划分
x_train = x_set2[0:200]
y_train = y_set2[0:200,5]
print(y_train.shape)
print(x_train.shape)
x_test = x_set2[200:]
y_test = y_set2[200:,5]
...
y_set2 = np.array(y_set1)
x_train = x_set2[0:500]
y_train = y_set2[0:500]
x_test = x_set2[500:]
y_test = y_set2[500:]'''

def acc_top9(y_true, y_pred):
    return keras.metrics.top_k_categorical_accuracy(y_true, y_pred, k=9)
...
model.compile(optimizer='adam', loss=binary_focal_loss(gamma=2, alpha=0.25), metrics=['binary_accuracy']) ###,'top_k_categorical_accuracy'])#metrics.hanming_loss
...
plt.ylabel('train_accuracy_Value')
plt.xlabel('Epoch')
plt.title('Model training')
...
plt.ylabel('Value')
plt.xlabel('Epoch')
plt.legend(['train_loss', 'train_evaluate'], loc='upper left')
plt.show()'''
# 绘制测试损失值和评估
'''plt.plot(history.history['val_loss'])
plt.plot(history.history['val_binary_accuracy'])
